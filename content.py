...
MATH_REFERENCE = [
    {"name":"Mean","formula":"x̄ = Σ xᵢ / n","explanation":"The arithmetic average of observed values.","variables":"xᵢ = the i-th observed value; n = number of observations; Σ = summation across all observations; x̄ = sample mean."},
    {"name":"Median","formula":"Odd n: x̃ = x₍ₙ₊₁₎/₂  |  Even n: x̃ = (x₍ₙ/₂₎ + x₍ₙ/₂₊₁₎) / 2","explanation":"The middle value after observations are sorted. It is less sensitive to extreme values than the mean.","variables":"x̃ = sample median; x₍j₎ = the j-th value after sorting; n = number of observations."},
    {"name":"Sample variance","formula":"s² = Σ(xᵢ − x̄)² / (n − 1)","explanation":"Measures squared dispersion around the sample mean.","variables":"s² = sample variance; xᵢ = the i-th observation; x̄ = sample mean; n = sample size; Σ = summation across observations."},
    {"name":"Sample standard deviation","formula":"s = √s²","explanation":"Expresses sample variation in the original measurement units.","variables":"s = sample standard deviation; s² = sample variance; √ = square-root operation."},
    {"name":"Z-score","formula":"z = (x − μ) / σ","explanation":"Expresses how many population standard deviations an observation is from the population mean.","variables":"z = standardized score; x = observed value; μ = population mean; σ = population standard deviation."},
    {"name":"Standard error of the mean","formula":"SE = s / √n","explanation":"Estimates the sampling variability of the sample mean.","variables":"SE = standard error of the sample mean; s = sample standard deviation; n = sample size; √ = square-root operation."},
    {"name":"95% confidence interval for a mean","formula":"x̄ ± t* × SE","explanation":"Provides an interval estimate for a population mean using a specified confidence level.","variables":"x̄ = sample mean; t* = critical t value for the selected confidence level and degrees of freedom; SE = standard error of the mean; ± = plus or minus."},
    {"name":"Binomial probability","formula":"P(X = k) = C(n, k) pᵏ (1 − p)ⁿ⁻ᵏ","explanation":"Models the probability of exactly k successes in n independent trials with a constant success probability.","variables":"P(X = k) = probability of exactly k successes; X = number of successes; k = specified number of successes; n = number of trials; p = probability of success on each trial; C(n,k) = combinations of n items taken k at a time."},
    {"name":"Poisson probability","formula":"P(X = k) = e⁻λ λᵏ / k!","explanation":"Models the probability of k events occurring in a fixed interval when Poisson assumptions are appropriate.","variables":"P(X = k) = probability of k events; X = event count; k = specified event count; λ = expected event count in the interval; e = Euler's number; k! = factorial of k."},
    {"name":"Defects per opportunity","formula":"DPO = defects / (units × opportunities per unit)","explanation":"Normalizes observed defects by the number of opportunities for a defect to occur.","variables":"DPO = defects per opportunity; defects = total defects observed; units = number of units processed; opportunities per unit = defined defect opportunities for each unit; × = multiplication."},
    {"name":"Defects per million opportunities","formula":"DPMO = DPO × 1,000,000","explanation":"Expresses defects per opportunity on a one-million-opportunity basis.","variables":"DPMO = defects per million opportunities; DPO = defects per opportunity; 1,000,000 = one million opportunities."},
    {"name":"Yield","formula":"Yield = good units / total units","explanation":"The proportion of units that meet the defined acceptance rule.","variables":"Yield = proportion of conforming units; good units = units meeting the acceptance rule; total units = all units evaluated."},
    {"name":"Cp","formula":"Cp = (USL − LSL) / (6σ)","explanation":"Compares specification width with six standard deviations of process variation, without considering process centering.","variables":"Cp = potential capability index; USL = upper specification limit; LSL = lower specification limit; σ = process standard deviation; 6 = six-standard-deviation capability width."},
    {"name":"Cpk","formula":"Cpk = min[(USL − μ)/(3σ), (μ − LSL)/(3σ)]","explanation":"Measures capability while accounting for the location of the process mean within the specification limits.","variables":"Cpk = centered capability index; USL = upper specification limit; LSL = lower specification limit; μ = process mean; σ = process standard deviation; min = the smaller one-sided capability value."},
    {"name":"Simple linear regression","formula":"Y = β₀ + β₁X + ε","explanation":"Models a response as a linear function of one predictor plus an error term.","variables":"Y = response variable; X = predictor variable; β₀ = intercept; β₁ = slope coefficient for X; ε = error term not explained by the model."},
    {"name":"Multiple linear regression","formula":"Y = β₀ + Σ(βⱼXⱼ) + ε","explanation":"Models a response using several predictors.","variables":"Y = response variable; β₀ = intercept; βⱼ = coefficient for predictor j; Xⱼ = value of predictor j; Σ = sum across predictors; ε = error term."},
    {"name":"CUSUM","formula":"Cₜ = max(0, Cₜ₋₁ + xₜ − μ₀ − k)","explanation":"Accumulates deviations from a target to detect sustained shifts. Exact one-sided or two-sided forms depend on chart design.","variables":"Cₜ = cumulative statistic at time t; Cₜ₋₁ = previous cumulative statistic; xₜ = current observation; μ₀ = target or in-control mean; k = reference value; t = observation time/index; max(0,·) = retain zero rather than a negative cumulative value in this one-sided form."},
    {"name":"EWMA","formula":"Zₜ = λxₜ + (1 − λ)Zₜ₋₁","explanation":"Weights recent observations more heavily while retaining information from earlier observations.","variables":"Zₜ = EWMA statistic at time t; Zₜ₋₁ = previous EWMA statistic; xₜ = current observation; λ = smoothing constant between 0 and 1; t = observation time/index."},
    {"name":"Correlation","formula":"r = cov(X,Y) / (sₓ sᵧ)","explanation":"Standardizes linear association between two variables.","variables":"r = sample correlation coefficient; cov(X,Y) = sample covariance of X and Y; sₓ = sample standard deviation of X; sᵧ = sample standard deviation of Y."},
    {"name":"R-squared","formula":"R² = 1 − SSE / SST","explanation":"Describes the proportion of sample response variation explained by the fitted regression model.","variables":"R² = coefficient of determination; SSE = sum of squared errors; SST = total sum of squares."},
    {"name":"Full factorial combinations","formula":"Number of combinations = 2ᵏ","explanation":"A two-level full factorial experiment with k factors contains 2ᵏ treatment combinations.","variables":"k = number of factors; 2 = number of levels per factor; 2ᵏ = number of treatment combinations."},
    {"name":"PCA variance share","formula":"Variance shareⱼ = eigenvalueⱼ / Σ eigenvalues","explanation":"Shows the share of total variance represented by a principal component.","variables":"Variance shareⱼ = proportion of variance represented by component j; eigenvalueⱼ = eigenvalue for component j; Σ eigenvalues = sum of all retained eigenvalues."},
    {"name":"Net present value","formula":"NPV = Σ[CFₜ / (1 + r)ᵗ] − initial investment","explanation":"Converts future project cash flows into present value and subtracts the initial investment.","variables":"NPV = net present value; CFₜ = net cash flow in period t; r = discount rate per period; t = period number; Σ = sum across forecast periods; initial investment = cash paid at project start."},
]
